...
with tab_scan:
    ...
    if results:
        ...
        tooltips = pd.DataFrame("", index=df_view.index, columns=df_view.columns)
        if "Score scale" in df_view.columns:
            tooltips["Score scale"] = df_view.apply(_scale_tooltip, axis=1)
        ...
        if "Score scale" in df_view.columns:
            styler = styler.applymap(_color_scale, subset=["Score scale"]).set_tooltips(tooltips)
